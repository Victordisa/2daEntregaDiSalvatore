...
while (comprar != "no") {
    ...
    let precioConIvaYenvio = carrito.map((el) =>{
        return {
            precio: el.getPrecioConEnvio() *1.21
            
        }
    })
    total = precioConIvaYenvio.reduce((acc, el) => {
        acc += el.precio;
        return acc
    }, 0);
...
console.log("Ud debera pagar un precio total de" + " " + total.toFixed(2));
